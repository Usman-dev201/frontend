...
export default function ListLedgerEntries() {
  ...
  const fetchLedgerEntries = useCallback(async () => {
    try {
      const response = await api.get("/LedgerEntry/assets");
      setLedgerEntries(response.data || []);
    } catch (error) {
      console.error("Error fetching asset ledger entries:", error);
      alert("Failed to load asset ledger entries.");
    }
  }, []);
  ...
  const filteredData = useMemo(() => {
    ...
    return ledgerEntries.filter(
      (l) =>
        ...
        l.location?.locationName?.toLowerCase().includes(q)
    );
