...
import "./Assets.css"; // Reuse same stylesheet

export default function ListExpenseLedgerEntries() {
  ...
  const fetchLedgerEntries = useCallback(async () => {
    try {
      const response = await api.get("/LedgerEntry/expenses");
      setLedgerEntries(response.data || []);
    } catch (error) {
      console.error("Error fetching expense ledger entries:", error);
      alert("Failed to load expense ledger entries.");
    }
  }, []);
  ...
  const columnsRef = useRef([
    ...
    {
      header: "Location", // ✅ Added Location column
