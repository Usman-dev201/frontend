...
import "./Assets.css"; // Reuse same CSS

export default function ListRevenueLedgerEntries() {
  ...
  const fetchLedgerEntries = useCallback(async () => {
    try {
      const response = await api.get("/LedgerEntry/revenues");
      setLedgerEntries(response.data || []);
    } catch (error) {
      console.error("Error fetching revenue ledger entries:", error);
      alert("Failed to load revenue ledger entries.");
    }
  }, []);
